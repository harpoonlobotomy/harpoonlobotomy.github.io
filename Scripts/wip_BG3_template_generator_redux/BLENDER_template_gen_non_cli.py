## Mapping fixed. All works, at least for this specific json. Will need to amend the nodegroups doc as I go.
## But, for now, this one instances all nodes and nodegroups, and all links are made correctly.
#31/7/25
...
MAIN_JSON_PATH = r"F:\test\wrapper_script_test_output3\stage_5_tmp_CHAR_Fur_3.json"
...
def set_reroute_type(node_instance,  node_tree):
    reroute = node_instance
    valuenode = node_tree.nodes.new("ShaderNodeValue")
    temp_nodes.add(valuenode)
    from1 = valuenode.outputs[0]
    to1 = reroute.inputs[0]

#    print(f"From 1 socket: {from1.bl_idname}, {from1.identifier}, type: {from1.bl_idname}, linked: {from1.is_linked}")
    node_tree.links.new(from1, to1)
#    print(f"From 1 socket: {from1.bl_idname}, {from1.identifier}, type: {from1.bl_idname}, linked: {from1.is_linked}")
...
def map_native_connectors(node_type, node_id, node_instance, connectors_json, connectionsock, node_tree):
    ...
    if node_type == "PassthroughNode":
        set_reroute_type(node_instance,  node_tree)
    ...
    try:
        ...
        #        print(f"Socket ID {socket_id} not used")
            ...
            key = None
            type = None
            key = conn.get("Socket_Identifier")
            if not key:
                print(f"[Warning] No identifier for connector {conn}")
                continue

            socket = next((sock for sock in relevant_sockets if sock.identifier == key), None)
            if socket:
    #            print(f"[Mapped] {socket_id} → {socket.name}, identifier={socket.identifier}")
                mapping[socket_id] = socket
            else:
                print(f"[Unmapped] Socket ID {socket_id} with identifier={key} not found in node {node_id}")

            if socket is None:
                print(f"[Warning] Blender socket not found for key '{key}' in {sock.identifier}, {sock.name}")
       #     if socket is not None:
            #    print(f"Found socket:  Name: {sock.name}, identifier: {sock.identifier}, type: {sock.type}")
...
def get_node_sets(nodes_json):
    ...
    optional_keys = [
        ...
        "Blender_Operation", "d2p1_Attributes", "Location", "Autohide"]
    ...
    for entry in nodegroup_entries:
        ...
        try:
            ...
            if ng_data:
                ...
                debug_print(["nodegroup_setup", "errors"], "No ng_data for nodegroup {node_type}")
        except Exception as e:
...
def create_nodegroup_from_ng_data(name, node_type, ng_data, node_id, connections_list):
    ...
    node_group["autohide"] = autohide
    debug_print("nodegroup_setup", f"autohide:", node_group.get("autohide", "not set"))
    debug_print("nodegroup_setup", f"autohide and color done")
...
def create_material_and_instance(blendernative_entries, nodegroup_entries, nodes_json, nodegroup_json_data, connections_list):
...
def create_blender_links(master_tree, connections_list, node_instance_map, global_socket_map, used_connectors):
    ...
    #    print(f"{conn}")
        if conn["from_socket"] not in used_connectors:
            print(f"[Unused] from_socket {conn['from_socket']} not mapped")
        if conn["to_socket"] not in used_connectors:
            print(f"[Unused] to_socket {conn['to_socket']} not mapped")
...
node_tree = create_material_and_instance(blendernative_entries, nodegroup_entries, nodes_json, nodegroup_json_data, connections_list)
